...
exports.createOrder = async (req, res) => {
  try {
    // Calcular o total com base nos itens do pedido
    const orderTotal = req.body.items.reduce((acc, item) => {
      // Certifique-se de que item.quantity e item.productId estão definidos
      if (item.quantity && item.productId) {
        // Obtenha o preço do produto (você pode precisar modificar isso com base na estrutura do seu modelo de produto)
    ...
    // Crie a nova ordem com o total calculado
